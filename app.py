...
#    지구본 아이콘이 뜨므로, 헤더에 이미 있는 로고 이미지를 그대로 내려줍니다.
@app.route('/favicon.ico')
def favicon():
    import re
    m = re.search(r'data:image/png;base64,([A-Za-z0-9+/=]+)', HTML_PAGE)
    if not m:
        return '', 404
    return Response(base64.b64decode(m.group(1)), mimetype='image/png')
